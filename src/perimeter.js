    ...
    if (C === 'M') {
      ...
      for (let i = 0; i + step - 1 < args.length; i += step) {
        ...
        push({
          kind: 'curve',
          ...flattenedWalker(poly),
          d: `Q${n(c1[0])} ${n(c1[1])} ${n(end[0])} ${n(end[1])}`,
        });
        prevQuadCtrl = c1;
        prevCubicCtrl = null;
        cur = end;
...
export function parseClipPath(value, box) {
  ...
  if (fn === 'circle') {
    ...
    const radius = isLen(r)
      ? resolveLen(r, Math.hypot(width, height) / Math.SQRT2)
  ...
  if (fn === 'ellipse') {
    ...
    return ellipseShape({
      cx: resolveLen(cxs ?? '50%', width),
      cy: resolveLen(cys ?? '50%', height),
      rx,
      ry,
    });
  }
...
function readShape(host, inset) {
  ...
  const hasPerCorner =
    cs.borderTopRightRadius != null ||
    cs.borderBottomRightRadius != null ||
    cs.borderBottomLeftRadius != null;
